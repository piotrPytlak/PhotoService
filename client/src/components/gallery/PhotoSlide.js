...
export function PhotoSlide() {
    ...
    const {serverUrl, userAllPhotos, photosInAlbum} = useContext(apiContext);
    ...
    const onChangeIndex = (x) => {
        ...
        console.log('xd:',albumId);
        history.push(`/photo/gallery/${userId}/${newPhoto.id}${albumId ? '/' + albumId : ''}`);

        JSON.stringify(dataPhoto) === JSON.stringify(newDataPhoto) && setSIndex(newIndex);
    ...
    const initialPhotoData = (photoList) => {
        ...
        const indexStart = currentIndex - 2 > 0 ? currentIndex - 2 : 0;
        const indexEnd = indexStart + 5 < photoList.length ? indexStart + 5 : photoList.length;
        ...
        userAllPhotos(userId).then((x) => {
            const photoList = x.map((photo) => {
                return {
                    ...
                    thumbnail: "https://lh3.googleusercontent.com/proxy/qNxKnVlotKLm8f251E4w0OV9G-2F3RqsUv29FtDek6m30M151m82n3by5r9ax9QWPADTjLg0OR30muBcxxUk-RFSePIf2SM",
                    title: photo.photoTitle,
                    description: photo.Description,
                };
            });
            setPhotos(photoList);
            initialPhotoData(photoList);
            return photoList;
        })
    };


    const photosFromAlbum = () => {
    ...
    useEffect(() => {
        ...
        albumId ? photosFromAlbum() : allPhotos();
    ...
    return (dataPhoto.length > 0 && <div ref={lightBox} className={"slideContainer"}>
            <LightBox
                state={toggle}
                ...
                imageIndex={sIndex}
            />
            {isClosed && <Redirect to={`/${albumId ? 'album' : 'photos'}/user/${userId}${albumId ? '/' + albumId : ''}`}/>}
        </div>);
}
